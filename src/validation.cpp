...
#if defined(NDEBUG)
# error "Eleccoin cannot be compiled without assertions."
#endif
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, m_limit_descendants, m_limit_descendant_size, errString)) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/eleccoin-dev/2018-November/016518.html
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ECC additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Subsidy is cut off 25% every 88,000 blocks which will occur approximately every 1 years.
    // 2200m coins max by year 2080.    
    for (int i = 0; i < halvings; i++) {
        nSubsidy -= (0.2 * nSubsidy);
    }
