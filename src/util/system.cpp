...
// For details see https://github.com/eleccoin/eleccoin/pull/22348.
...
const char * const ELECCOIN_CONF_FILENAME = "eleccoin.conf";
const char * const ELECCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/eleccoin/eleccoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //eleccoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ELECCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Eleccoin
    // macOS: ~/Library/Application Support/Eleccoin
    // Unix-like: ~/.eleccoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Eleccoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Eleccoin";
#else
    // Unix-like
    return pathRet / ".eleccoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ELECCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Eleccoin Core copyright is not removed by accident
    if (copyright_devs.find("Eleccoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Eleccoin Core developers";
    }
