...
{
    DifficultyMode dmode = DIFFICULTY_DGW;

    if (pindexLast->nHeight + 1 >= params.nDGWActivationHeight)
    {
        dmode = DIFFICULTY_DGW;
    }
    else
    {
        dmode = DIFFICULTY_ENB;
    }

    unsigned int req = 0;

    if (dmode == DIFFICULTY_DGW)
    {
        req = GetNextWorkRequiredDarkGravityWave(pindexLast, pblock, params);
    }
    else
    {
        req = GetNextWorkRequiredEveryNBlocks(pindexLast, pblock, params);
    }        
