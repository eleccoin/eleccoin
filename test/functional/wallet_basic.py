...
class WalletTest(EleccoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 21 ECC from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 100 ecc in block rewards plus fees, but
        ...
        # Send 10 ECC with subtract fee from amount
        ...
        # Sendmany 10 ECC with subtract fee from amount
        ...
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_ecc_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.sync_all(self.nodes[0:3])
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_ecc_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid Eleccoin address
            assert_raises_rpc_error(-5, "Invalid Eleccoin address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid Eleccoin address
            assert_raises_rpc_error(-5, "Invalid Eleccoin address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid Eleccoin address or script", self.nodes[0].importaddress, "invalid")
            ...
            assert_raises_rpc_error(-5, "Bech32m addresses cannot be imported into legacy wallets", self.nodes[0].importaddress, "ecrt1p0xlxvlhemja6c4dqv22uapctqupfhlxm9h8z3k2e72q4k9hcz7vqc8gma6")
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_ecc_kvb))
