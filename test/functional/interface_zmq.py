...
from test_framework.address import (
    ADDRESS_ECRT1_P2WSH_OP_TRUE,
    ADDRESS_ECRT1_UNSPENDABLE,
)
...
class ZMQTest (EleccoinTestFramework):
    ...
    def test_basic(self):
        ...
        genhashes = self.nodes[0].generatetoaddress(num_blocks, ADDRESS_ECRT1_UNSPENDABLE)
        ...
        if self.is_wallet_compiled():
            ...
            self.nodes[0].generatetoaddress(1, ADDRESS_ECRT1_UNSPENDABLE)
            hashtx.receive()
            txid = hashtx.receive()
            assert_equal(payment_txid, txid.hex())
    ...
    def test_reorg(self):
        ...
        disconnect_block = self.nodes[0].generatetoaddress(1, ADDRESS_ECRT1_UNSPENDABLE)[0]
        ...
        connect_blocks = self.nodes[1].generatetoaddress(2, ADDRESS_ECRT1_P2WSH_OP_TRUE)
    ...
    def test_sequence(self):
        ...
        [seq] = self.setup_zmq_test([("sequence", "tcp://127.0.0.1:29333")])
        self.disconnect_nodes(0, 1)
        ...
        dc_block = self.nodes[0].generatetoaddress(1, ADDRESS_ECRT1_UNSPENDABLE)[0]

        # Note: We are not notified of any block transactions, coinbase or mined
        assert_equal((self.nodes[0].getbestblockhash(), "C", None), seq.receive_sequence())

        # Generate 2 blocks in nodes[1] to a different address to ensure a chain split
        self.nodes[1].generatetoaddress(2, ADDRESS_ECRT1_P2WSH_OP_TRUE)
        ...
        if self.is_wallet_compiled():
            ...
            c_block = self.nodes[0].generatetoaddress(1, ADDRESS_ECRT1_UNSPENDABLE)[0]
            self.sync_all()
            ...
            self.nodes[1].generatetoaddress(1, ADDRESS_ECRT1_UNSPENDABLE)
            self.sync_all()
