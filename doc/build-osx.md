...
This guide describes how to build eleccoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Eleccoin Core from source.
...
### 4. Clone Eleccoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Eleccoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `eleccoind` or  `eleccoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/eleccoin/eleccoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Eleccoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/eleccoin/eleccoin/issues/7714).
...
You can deploy a `.dmg` containing the Eleccoin Core application using `make deploy`.
...
Run the following in your terminal to compile Eleccoin Core:
...
## Running Eleccoin Core

Eleccoin Core should now be available at `./src/eleccoind`.
If you compiled support for the GUI, it should be available at `./src/qt/eleccoin-qt`.

The first time you run `eleccoind` or `eleccoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Eleccoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Eleccoin"

touch "/Users/${USER}/Library/Application Support/Eleccoin/eleccoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Eleccoin/eleccoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Eleccoin/debug.log
```

## Other commands:

```shell
./src/eleccoind -daemon      # Starts the eleccoin daemon.
./src/eleccoin-cli --help    # Outputs a list of command-line options.
