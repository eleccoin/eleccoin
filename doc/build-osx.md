...
1. Clone the Eleccoin Core source code:
    ```shell
    git clone https://github.com/eleccoin/eleccoin
    cd eleccoin
    ```

2.  Build Eleccoin Core:

    Configure and build the headless Eleccoin Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, Eleccoin Core may be
...
## Running
Eleccoin Core is now available at `./src/eleccoind`
...
The first time you run eleccoind, it will start downloading the blockchain. This process could
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/eleccoin-core/eleccoin-detached-sigs).
