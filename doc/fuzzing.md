# Fuzzing Eleccoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Eleccoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/eleccoin/eleccoin
$ cd eleccoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/eleccoin/eleccoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of Eleccoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/eleccoin_report.pdf) includes an external overview of the status of Eleccoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/eleccoin/eleccoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/eleccoin/eleccoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/eleccoin/eleccoin/tree/master/src/test/fuzz).
...
It is possible to specify `eleccoind` arguments to the `fuzz` executable.
...
with certain assembly code from Eleccoin Core's code. See [developer notes on sanitizers](https://github.com/eleccoin/eleccoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Eleccoin Core using afl++

## Quickstart guide

To quickly get started fuzzing Eleccoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Eleccoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Eleccoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Eleccoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Eleccoin
Core without having to write any custom fuzzing harness. The `eleccoind` server
...
```sh
$ mkdir eleccoin-honggfuzz-p2p/
$ cd eleccoin-honggfuzz-p2p/
$ git clone https://github.com/eleccoin/eleccoin
$ cd eleccoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/eleccoind.cpp b/src/eleccoind.cpp
...
EOF
$ make -C src/ eleccoind
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=19844 -f inputs/ -- \
          src/eleccoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:19844 -logthreadnames \
                       -debug
```

# Fuzzing Eleccoin Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing Eleccoin Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/eleccoin/eleccoin/pull/22472) for more information.
...
Eleccoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/eleccoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=eleccoin-core).
...
because Eleccoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_eleccoin-core/latest).
