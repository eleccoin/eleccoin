# CJDNS support in Eleccoin Core

It is possible to run Eleccoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Eleccoin Core, similarly to a VPN (either in the host/OS or on
the network router).
...
redundancy and robustness for both the Eleccoin network and individual nodes.
...
## Run Eleccoin Core with CJDNS

Once you are connected to the CJDNS network, the following Eleccoin Core
...
When enabled, this option tells Eleccoin Core that it is running in an
...
IPv6 local network. This helps Eleccoin Core perform better address management:
...
CJDNS support was added to Eleccoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `eleccoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/eleccoin/eleccoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Eleccoin Core

There are several ways to see your CJDNS address in Eleccoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `eleccoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `eleccoin-cli getpeerinfo`).
