# User-space, Statically Defined Tracing (USDT) for Eleccoin Core

Eleccoin Core includes statically defined tracepoints to allow for more
...
**Note**: Eleccoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Eleccoin Core
...
Multiple tools can list the available tracepoints in a `eleccoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Eleccoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/eleccoind
stap net        outbound_message 0x0000000000107c05 /src/eleccoind
stap validation block_connected  0x00000000002fb10c /src/eleccoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Eleccoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/eleccoind | grep NT_STAPSDT -A 4 -B 2
