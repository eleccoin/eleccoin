# PSBT Howto for Eleccoin Core

Since Eleccoin Core 0.17, an RPC interface exists for Partially Signed Eleccoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/eleccoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Eleccoin transactions that are not fully signed
...
Overall, the construction of a fully signed Eleccoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Eleccoin transaction (in network format)
...
#### Multisig with multiple Eleccoin Core instances
...
Eleccoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `eleccoin-cli` is used.
...
Later, when *V* ECC has been received on *Amulti*, and Bob and Carol want to
