...
Eleccoin Core version 1.2.0 is now available from:
...
#### Removal of EIP61 reject network messages from Eleccoin Core

The `-enableeip61` command line option to enable EIP61 has been removed.

This feature has been disabled by default since Eleccoin Core version 1.1.0.
...
- Testing or debugging of implementations of the Eleccoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Eleccoin Core. Eleccoin Core logs debug messages
...
The removal of EIP61 reject message support also has the following minor RPC
...
- The `walletprocesspsbt` and `walletcreatefundedpsbt` RPCs now include
  EIP32 derivation paths by default for public keys if we know them.
  This can be disabled by setting the `eip32derivs` parameter to
  `false`.  (#17264)

- The `bumpfee` RPC's parameter `totalFee`, has been removed.  (#18312)
...
- OpenSSL is no longer used by Eleccoin Core.  (#17265)

- EIP70 support has been fully removed from Eleccoin Core. The
  `--enable-eip70` option remains, but it will throw an error during configure.
  (#17165)
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/eleccoin/eleccoin/issues/16599),
  [PR #16702](https://github.com/eleccoin/eleccoin/pull/16702), and the
  `eleccoind help` for more information.  This option is experimental and
...
- All custom settings configured when Eleccoin Core starts are now
...
- The `-whitelistforcerelay` configuration parameter has been removed after
  it was discovered that it was rendered ineffective and
  hasn't actually been supported for almost four years.  (#17985)

GUI changes
-----------

- The "Start Eleccoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" in the release notes of version 1.1.0 has been fixed.  The issue
  affected anyone who simultaneously used multiple Eleccoin Core wallets
...
- Eleccoin Core's automatically-generated source code documentation is
  now available at https://doxygen.eleccoincore.org.  (#17596)
...
- The `eleccoin-cli` utility used with the `-getinfo` parameter now
...
- The `eleccoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=9833` `-test.server=1` previously were allowed but ignored. Now
...
- #15437 Remove EIP61 reject messages (MarcoFalke)
...
- #17264 Set default eip32derivs to true for psbt methods (Sjors)
...
- #17068 Always generate `eleccoinstrings.cpp` on `make translate` (D4nte)
...
- #17165 Remove EIP70 support (fanquake)
...
- #18360 Bump transifex slug and update English translations for 1.2.0 (laanwj)
...
- #17741 Included `test_eleccoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBELECCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_eleccoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBELECCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for eleccoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate eleccoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest eleccoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_eleccoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (eleccoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18334 Add basic test for EIP 37 (MarcoFalke)
...
- #18430 ci: Only clone eleccoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #18481 add EIP37 'filterclear' test to p2p_filter.py (theStack)
...
- #18515 Add EIP37 remote crash bug [CVE-2013-5700] test to `p2p_filter.py` (theStack)
...
- #16983 Add detailed info about Eleccoin Core files (hebasto)
- #16986 Doxygen-friendly CuckooCache comments (ch4ot1c)
- #17022 move-only: Steps for "before major release branch-off" (MarcoFalke)
- #17026 Update eips.md for default bech32 addresses in 1.2.0 (MarcoFalke)
...
- #17119 Fix broken eleccoin-cli examples (andrewtoth)
...
- #17648 Rename wallet-tool references to eleccoin-wallet (hel-o)
...
- #18341 Replace remaining literal ECC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On eleccoind startup, write config args to debug.log (LarryRuane)
...
- #17049 contrib: Bump gitian descriptors for 1.2.0 (MarcoFalke)
...
- #17650 util: Remove unwanted fields from eleccoin-cli -getinfo (malevolent)
