...
- [Development guidelines](#development-guidelines)
    - [General Eleccoin Core](#general-eleccoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Eleccoin Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
with "play eleccoins" on a test network.
...
Eleccoin Core is a multi-threaded application, and deadlocks or other
...
CPPFLAGS, e.g. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run eleccoind.

You can then use the `-debug=lock` configuration option at eleccoind startup or
`eleccoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `eleccoin-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/eleccoin/eleccoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_eleccoin
...
# A coverage report will now be accessible at `./test_eleccoin.coverage/index.html`.
```
...
To profile a running eleccoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep eleccoind` -- sleep 60
```
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/eleccoin/eleccoin/issues/12691)
...
However, in open source software such as Eleccoin Core, where everyone uses
...
pay attention to for reviewers of Eleccoin Core code.

General Eleccoin Core
----------------------
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `ELECCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef ELECCOIN_FOO_BAR_H
#define ELECCOIN_FOO_BAR_H
...
#endif // ELECCOIN_FOO_BAR_H
```
...
Some of these are maintained by active developers of Eleccoin Core, in which case
...
a Eleccoin Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/eleccoin/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/eleccoin/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/eleccoin/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/eleccoin/ctaes ; maintained by Core contributors.

- src/univalue
  - Subtree at https://github.com/eleccoin/univalue-subtree ; maintained by Core contributors.
...
file descriptors, it will cause problems with Eleccoin's `select()` loop, because
...
compatibility between nodes. This happened in Eleccoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Eleccoin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `eleccoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `eleccoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Eleccoin Core, the wallet was always
