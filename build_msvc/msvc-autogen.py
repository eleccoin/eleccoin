...
def parse_config_into_ecc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Eleccoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/eleccoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/eleccoin_config.h'), "w", encoding="utf8") as ecc_config:
        ecc_config.writelines(template)

def main():
    parser = argparse.ArgumentParser(description='Eleccoin-core msbuild configuration initialiser.')
        ...
        with open(vcxproj_filename + '.in', 'r', encoding='utf-8') as vcxproj_in_file:
            with open(vcxproj_filename, 'w', encoding='utf-8') as vcxproj_file:
                vcxproj_file.write(vcxproj_in_file.read().replace(
                    '@SOURCE_FILES@\n', content))
    parse_config_into_ecc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/eleccoin_config.h'), os.path.join(SOURCE_DIR, 'config/eleccoin-config.h'))
