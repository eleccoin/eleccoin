...
WORKINGDIR = "/tmp/eleccoin_verify_binaries"
HASHFILE = "hashes.tmp"
...
def main(args):
    ...
    if not success:
        print("eleccoin.org failed to provide signature file, "
              "but eleccoincore.org did?")
        print("wget output:")
        print(indent(output, '\t'))
    ...
    if not files_are_equal(sigfile1, sigfile2):
        print("eleccoin.org and eleccoincore.org signature files were not equal?")
    ...
    if retval != 0:
        if retval == 1:
            ...
            print("gpg error. Do you have the Eleccoin Core binary release "
